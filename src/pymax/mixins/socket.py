    ...
    def _unpack_packet(self, data: bytes) -> dict[str, Any] | None:
        ...
        if payload_bytes:
            if comp_flag != 0:
                ...
                try:
                    # Increasing uncompressed_size to 10MB to support larger packets
                    payload_bytes = lz4.block.decompress(
                        compressed_data,
                        uncompressed_size=10 * 1024 * 1024,
                    )
                except lz4.block.LZ4BlockError:
                    self.logger.error("LZ4 decompression failed: payload too large or corrupted")
                    return None
...
===============================================================
    ...
    async def _recv_loop(self) -> None:
        ...
        while True:
            try:
                ...
                self.logger.error("Error in recv_loop: %s; exiting loop for reconnect", e)
                self.is_connected = False
                break
    ...
    async def _send_and_wait(
        ...
        try:
            ...
            self.logger.warning("Connection lost (err=%s), notifying start() for reconnect...", conn_err)
            self.is_connected = False
