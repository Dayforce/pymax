...
class AuthMixin(ClientProtocol):
    ...
    async def _login(self) -> None:
        ...
        self.logger.debug(f"Login response keys: {list(login_resp.keys())}")
        if "tokenAttrs" in login_resp:
             self.logger.debug(f"Token attrs keys: {list(login_resp['tokenAttrs'].keys())}")
